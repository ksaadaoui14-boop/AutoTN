...
class Auth_core extends CI_Controller {
	...
	function currentpass_check($str)
	{
		...
		$query = $this->auth_model->check_login($user_name,'result');
		...
		{					
			...
			if($this->bcrypt->check_password($str,$row->password))
			{
				$ok=1;
			}
		}
	...
	function login()
	{
		$this->form_validation->set_rules('username',lang_key('username'),'required|xss_clean');
		$this->form_validation->set_rules('password',lang_key('password'),'required|xss_clean');
		...
		else
		{
			...
			{					
				...
				if($row->user_type!=1 && $row->confirmed!=1)
					$ok=2;				
    ...
    function useremail_check_signup($str)
    {
        ...
        if ($res > 0)
        {
            $this->form_validation->set_message('useremail_check_signup', lang_key('email_already_in_use'));
            return FALSE;
        }
        else
        {
    ...
    function username_check_signup($str)
    {
        ...
        if ($res > 0)
        {
            $this->form_validation->set_message('username_check_signup', lang_key('username_already_in_use'));
            return FALSE;
        }
        else
        {
