...
class Payment_core extends MX_Controller {
	...
	public function chooserenewpackage($post_id) 
	{
		...
		$data['content'] 		= load_view('choose_package_view',$value,TRUE);
		$data['alias']	    	= 'choose_renew_package';
		load_template($data,$this->active_theme);
	}
...

		    	{
...

		    		{
						...
						if(isset($custom_val['renew']) && $custom_val['renew']=='renew') { // if the payment was done to renew the post package we don't check for admin approval since it was approved previously
			    			...
			    			if($package->featured_expiration_time>0)
			    			{
								$featured_expirtion_date  	= strtotime('+'.$package->featured_expiration_time.' days',$time);
				    			$featured_expirtion_date 	= mdate($datestring, $featured_expirtion_date);
			    				$data['featured'] = 1;
				    			$data['featured_expiration_date'] 	= $featured_expirtion_date;
			    			}
...

		    {
...

		    	{
...

		    		{
		    			...
		    			$expirtion_date  = strtotime('+'.$package->featured_expiration_time.' days',$time);
		    			...
		    			if(isset($custom_val['renew']) && $custom_val['renew']=='renew') {
		    				...
		    				if($this->post_model->increment_featured_date($package->featured_expiration_time,$post_id)==TRUE)
