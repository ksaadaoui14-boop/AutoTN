...
/**
 * Carbiz account Controller
 *
 * This class handles user account related functionality
 *
 * @package		Account
 * @subpackage	Account
 * @author		wKarim Saadaoui
...
class Account_core extends CI_Controller {
...

	{
...

		{
			...
			$query = $this->auth_model->check_login($this->input->post('useremail'),'result');
	...
	{
		if(get_settings('content_settings','enable_signup','Yes')=='No')
		{
			redirect(site_url());
		}
    ...
    public function signupform()
    {
    	...
    	{
    		if(get_settings('content_settings','enable_pricing','Yes')=='Yes')
	...
	function newaccount($type='',$user_type='car')
	{
		if(get_settings('content_settings','enable_signup','Yes')=='No')
		{
			redirect(site_url());
	...
	function register()
	{
		...
		$enable_pricing = get_settings('content_settings','enable_pricing','Yes');
	...
	public function useremail_check($str)
	{
		...
		if ($res>0)
		{
			$this->form_validation->set_message('useremail_check', lang_key('email_already_in_use'));
			return FALSE;
		}
		else
		{
	...
	public function username_check($str)
	{
		...
		if ($res>0)
		{
			$this->form_validation->set_message('username_check', lang_key('username_already_in_use'));
			return FALSE;
		}
		else
		{
...
public function get_admin_email_and_name()
{
    ...
    if(!empty($values) && is_object($values))
    {
        $data['admin_email'] = !empty($values->webadmin_email) ? $values->webadmin_email : 'admin@'.$_SERVER['HTTP_HOST'];
        $data['admin_name']  = !empty($values->webadmin_name)  ? $values->webadmin_name  : 'Admin';
    }
    else
    {
...
$this->load->library('email');
    ...
    $this->email->from($from, 'Test Sender');
    $this->email->to('your_target_email@example.com'); // حط بريدك
    $this->email->subject('SMTP Test');
    $this->email->message('<h3>It works! ✅</h3>');

    if ($this->email->send()) {
        echo 'Email sent!';
    } else {
        echo nl2br($this->email->print_debugger());
    }
}
