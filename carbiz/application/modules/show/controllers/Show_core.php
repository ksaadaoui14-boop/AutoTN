...
class Show_core extends CI_controller {
    ...
    public function get_city_val_dropdown_by_parent_ajax($parent='',$type='city')
    {
    	if($parent=='')
    	{
	    	echo '<option value="">'.lang_key('select_'.$type).'</option>';
    ...
    {    	
        ...
        $value['pages']			= configPagination('users',$total,3,$this->PER_PAGE);
	...
	public function location()
	{
		...
		if(get_settings('content_settings','disable_location','no')=='yes')
		{
			$this->output->set_status_header('404');
			$data['content'] 	= load_view('404_view','',TRUE);
	        load_template($data,$this->active_theme,'template_view');	
	...
	public function get_min_max_car_mileage_ajax() {
		$min_mileage = get_settings('content_settings','min_car_mileage','NO');
		$max_mileage = get_settings('content_settings','max_car_mileage','NO');
		if($min_mileage=='NO'||$min_mileage==null) {
			$min_mileage = 0;
		}
		if($max_mileage=='NO'||$max_mileage==null) {
			$max_mileage = 500000000;
		}
		echo json_encode(array('min_mileage'=>$min_mileage,'max_mileage'=>$max_mileage));
