...

{
	...
	function get_category_featured_img($cat_id)
	{
		$this->db->where('id',$cat_id);
		$query = $this->db->get('categories');
		if($query->num_rows()>0){
			$cat = $query->row();
			if($cat->featured_img!='')
				return base_url('uploads/car-icons/'.$cat->featured_img);
			else
				return base_url('uploads/car-icons/default.png');
		}
		return base_url('uploads/car-icons/default.png');
	}

	function check_url_slug($slug,$id='')
	{
		$slug = urlencode(trim($slug));
		$query = $this->db->get_where('posts',array('unique_id'=>$slug));
		if($query->num_rows()>0)
		{
			$row = $query->row();
			if($row->id==$id)
	...
	function get_recent_posts($limit=6) {
		...
		$order = get_settings('content_settings','recent_posts_order','DESC');
		$this->db->where('status',1);
		$this->db->limit($limit);
	...
	function get_top_posts($limit=6) {
		//updated on version 1.8
		$order = get_settings('content_settings','recent_posts_order','DESC');
		$this->db->where('status',1);
		$this->db->limit($limit);
	...
	function get_featured_posts($limit=6) {
		//updated on version 1.8
		$order = get_settings('content_settings','featured_posts_order','DESC');
		$this->db->where('status',1);
	...
	function get_category_posts($limit=6,$category_id='') {
		//updated on version 1.8
		$order = get_settings('content_settings','default_posts_order','DESC');		
		$this->db->where('status',1);
