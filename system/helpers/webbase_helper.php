...
if ( ! function_exists('get_author_url'))
{
	...
	{		
		return 'http://webhelios.com/app/envato/';
	}
}
...
if ( ! function_exists('get_user_fullname_from_user'))
{
    function get_user_fullname_from_user($user)
    {
...
if ( ! function_exists('check_config'))
{
	...
	{
		$files = array('user_uploads','user_uploads/plugins','web_config','application/config/database.php');
		$error = '';
...
if ( ! function_exists('lang_key'))
{
	function lang_key($key='')
	{
		...
		else
		{
			...
			{
				if(!@file_exists(FCPATH."web_config/locals/".$curr_lang.'.yml'))
				{
					$file_name = $default_lang.'.yml';
				}
				else
			...
			$lang =  $CI->yaml->parse_file('./web_config/locals/'.$file_name);
...
if ( ! function_exists('get_current_lang'))
{
	function get_current_lang()
	{
		...
		if(!@file_exists(FCPATH."web_config/locals/".$lang.'.yml'))
		{
			if($lang=='admin')
			$lang = default_lang();
			else
...
if ( ! function_exists('truncate'))
{
	function truncate($s, $l, $e = '...', $isHTML = false){
		...
		return mb_substr($s, 0, $l = min(strlen($s),  $l + $i)) . (count($tags = array_reverse($tags)) ? '</' . implode('></', $tags) . '>' : '') . (strlen($s) > $l ? $e : '');
	}
}
...
if ( ! function_exists('create_log'))
{
	...
	{		
		...
		$domain = base_url();
		if (!strpos('-'.$domain, "http://localhost/") && !strpos('-'.$domain, ":8080"))
		{
			//set POST variables
			...
			{
				$CI->load->helper('file');
				$data = read_file('./web_config/local-data.conf');
				if($data=='')
				{
					$CI->session->set_flashdata('msg', '<div class="alert alert-danger">Please verify your purchase.</div>');
					redirect(site_url('admin/purchase/regdomain'));
...
if ( ! function_exists('load_page_local_data'))
{
	function load_page_local_data($alias='',$lang='')
	{
		$CI 	= get_instance();
		$CI->load->helper('file');
		$file_path = './web_config/locals-pages/'.$alias.'_'.$lang.'.html';

		$status = 0;
		$main_content = '';
		if(@file_exists($file_path))
		{
			$main_content = read_file('./web_config/locals-pages/'.$alias.'_'.$lang.'.html');			
			$status = 1;
		}
		...
		if(@file_exists($file_path))
		{
			$sidebar = read_file('./web_config/locals-pages/'.$alias.'_'.$lang.'_sidebar.html');
			$status = 1;			
		}
...
//added on version 1.9
if ( ! function_exists('send_post_published_email'))
{
	function send_post_published_email($data=array())
	{
		$CI 	= get_instance();
		...
		$CI->load->model('admin/system_model');
		$tmpl = $CI->system_model->get_email_tmpl_by_email_name('post_published_email');
		$subject = $tmpl->subject;
		$subject = str_replace("#username",$data['user_name'],$subject);
		$subject = str_replace("#post_link",$data['link'],$subject);
		$subject = str_replace("#webadmin",$admin_name,$subject);


		$body = $tmpl->body;
		$body = str_replace("#username",$data['user_name'],$body);
		$body = str_replace("#post_link",$data['link'],$body);
		$body = str_replace("#webadmin",$admin_name,$body);
...
if ( ! function_exists('translatedDate'))
{
	function translatedDate($time)
	{
		$month 	= date('M',$time);
		$day 	= date('d',$time);
		$year 	= date('Y',$time);

		return lang_key($month).' '.lang_key($day).', '.lang_key($year);
	}
}
//end
...
if ( ! function_exists('get_tinymce_translate_url'))
{
	function get_tinymce_translate_url()
	{
		$curr_lang 		= get_current_lang();
		$default_lang 	= default_lang();

		if(!@file_exists(FCPATH."assets/tinymce/langs/".$curr_lang.'.js'))
		{
			if(!@file_exists(FCPATH."assets/tinymce/langs/".$default_lang.'.js'))
			{
				$file_name = 'en.js';
			}
			else
			{										
				$file_name = $default_lang.'.js';
			}

		}
		else
		{										
			$file_name = $curr_lang.'.js';
		}

		return base_url('assets/tinymce/langs/'.$file_name);
	}
}

if ( ! function_exists('render_custom_field'))
{
	function render_custom_field($field,$value='')
	{
		$html = "";
		$class = (!empty($field->category_id))?"custom-fields cat-".$field->category_id:'custom-fields cat-all';

		if($field->type=="text")
		{
            $v = (set_value($field->name)!='')?set_value($field->name):$value;

			$html =	'<div class="form-group '.$class.'">'.
                    '<label class="col-md-3 control-label">'.lang_key($field->name).'</label>'.
                    '<div class="col-md-8">'.
                        '<input id="'.$field->name.'" type="text" name="'.$field->name.'" placeholder="'.lang_key($field->help_text).'" value="'.$v.'" class="form-control">'.
		...
		else if($field->type=="textarea")
		{
			...
			$html =	'<div class="form-group '.$class.'">'.
                    '<label class="col-md-3 control-label">'.lang_key($field->name).'</label>'.
                    '<div class="col-md-8">'.
                        '<textarea id="'.$field->name.'" name="'.$field->name.'" class="form-control" placeholder="'.lang_key($field->help_text).'">'.$v.'</textarea>'.
                        ''.form_error($field->name).''.
                    '</div>'.
                	'</div>';
		}
		else if($field->type=="richtext")
		{
			...
			$html =	'<div class="form-group '.$class.'">'.
                    '<label class="col-md-3 control-label">'.lang_key($field->name).'</label>'.
                    '<div class="col-md-8">'.
                        '<textarea id="'.$field->name.'" name="'.$field->name.'" class="form-control rich" placeholder="'.lang_key($field->help_text).'">'.$v.'</textarea>'.
                        ''.form_error($field->name).''.
                    '</div>'.
                	'</div>';
		}
		else if($field->type=="select")
		{
			...
			$options_html = '<option value="">'.lang_key($field->help_text).'</option>';
			foreach($options as $opt)
			{
				$sel = ($v==$opt)?'selected="selected"':'';
				$options_html .= '<option value="'.$opt.'" '.$sel.'>'.lang_key($opt).'</option>';
			}

			$html =	'<div class="form-group '.$class.'">'.
                    '<label class="col-md-3 control-label">'.lang_key($field->name).'</label>'.
                    '<div class="col-md-8">'.
                        '<select name="'.$field->name.'" class="form-control">'.$options_html.'</select>'.
                        ''.form_error($field->name).''.
                    '</div>'.
                	'</div>';
		...
		else if($field->type=="checkbox")
		{
			...
			foreach($options as $opt)
			{
				$chk = ($v==$opt)?'checked="checked"':'';
				$input_html .= '<label><input name="'.$field->name.'" type="checkbox" '.$chk.' value="'.$opt.'">&nbsp;'.lang_key($opt).'</label>&nbsp;';
			}

            $v = (set_value($field->name)!='')?set_value($field->name):'';
			$html =	'<div class="form-group '.$class.'">'.
                    '<label class="col-md-3 control-label">'.lang_key($field->name).'</label>'.
                    '<div class="col-md-8">'.
                        ''.$input_html.''.
		...
		else if($field->type=="radio")
		{
			...
			{
				$chk = ($v==$opt)?'checked="checked"':'';
				$input_html .= '<label><input name="'.$field->name.'" type="radio" '.$chk.' value="'.$opt.'">&nbsp;'.lang_key($opt).'</label>&nbsp;';
			}
